...
impl CertificateAuthority {
    ...
    pub fn load_ca(private: &str, cert: &str) -> Result<CertificateAuthority, Error> {
        let private_key_bytes = fs::read(private)?;
        ...
        let ca_cert_bytes = fs::read(cert)?;
        let ca_cert = pemfile::certs(&mut ca_cert_bytes.as_slice())
            .map_err(|_| Error::Decode)?; 
        let cert = rustls::Certificate(ca_cert[0].clone());
        let ca_res = CertificateAuthority::new(private_key,cert,String::from_utf8(ca_cert_bytes).unwrap(),1_000).unwrap();
        Ok(ca_res)
    }
